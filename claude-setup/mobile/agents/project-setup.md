...
## Step 4b: Configure Metro, Babel, and path aliases for the mobile app

These three files are **required** for the Expo app to bundle correctly — especially on
web and in a Turborepo monorepo. Without them, Metro cannot resolve workspace packages
and the `@/` alias fails at runtime.
...
// Look for modules in both the app's and the monorepo root's node_modules
...
Install the Babel path-alias plugin:
...
Update `apps/mobile/tsconfig.json` to declare the same paths for TypeScript:
...
## Step 18: Smoke-test both apps

Verify that backend and mobile both start without errors before reporting success.
Run each check and record the result. **Do not skip this step.**

### 18a. Backend — TypeScript compilation
...
Expected: `webpack` or `tsc` compilation succeeds, `dist/main.js` is created.

Verify:

```bash
ls apps/backend/dist/main.js && echo "BUILD OK" || echo "BUILD FAILED"
```

### 18c. Backend — health check (start server, ping, stop)
...
Ping the default NestJS health endpoint:

```bash
curl -sf http://localhost:3001/ && echo "BACKEND RUNNING" || echo "BACKEND NOT RESPONDING"
```
...
If the backend did not respond: check `apps/backend/.env` contains `DATABASE_URL` and
the database is reachable (`docker compose ps`). Do not proceed until this passes.

### 18d. Mobile — TypeScript compilation
...
Expected: no output (exit 0). Fix any errors before continuing.

### 18e. Mobile — Metro web bundle (dry-run)

```bash
cd apps/mobile
npx expo export --platform web --output-dir /tmp/expo-smoke-test 2>&1 | tail -30
```

Expected: `Export was successful` or similar success message. The output directory is
temporary and can be ignored.

If the bundle fails with "Module not found" or an import error, check:
- `metro.config.js` exists and sets `watchFolders` + `resolver.nodeModulesPaths`
- `babel.config.js` exists with `module-resolver` plugin
- The failing import path is correct (check `@/` alias resolves to `src/`)

### 18f. Record smoke-test results

Note pass/fail for each check — include in the final report.
...
Stack: React Native (Expo) + NestJS + Prisma

Expo workflow: managed / bare (reason: <detected signal or "default">)

Monorepo:
  apps/backend/         — NestJS API with Prisma scaffold
...
Design system:
  apps/mobile/src/theme/    — lightTheme, spacing, typography
...
Smoke tests:
  ...
  Backend health check: PASS / FAIL (http://localhost:3001/)
  Mobile typecheck:     PASS / FAIL
  Mobile web bundle:    PASS / FAIL

Skills verified: <present / missing>
CLAUDE.md: updated

⚠ Any FAIL above must be resolved before handing off to plan-feature.
