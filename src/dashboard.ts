...
import { readdirSync, readFileSync, existsSync, mkdirSync, writeFileSync, watch } from 'fs'
...
watch(INTERACTIONS_DIR, { persistent: false }, () => setTimeout(broadcastRuns, 100))
  ...
  if (req.method === 'POST' && req.url?.startsWith('/api/pipeline/respond/')) {
    ...
    req.on('end', () => {
      try {
        ...
        const responsePath = join(RESPONSES_DIR, `${threadId}-${interruptKey}.json`)
        writeFileSync(responsePath, JSON.stringify({ response, timestamp: new Date().toISOString() }))
        res.writeHead(200, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({ ok: true }))
      } catch (e: any) {
        res.writeHead(400, { 'Content-Type': 'application/json' })
