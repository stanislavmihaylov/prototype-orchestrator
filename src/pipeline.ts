...
function loadPipelineConfig(): Record<string, string> {
  ...
  try {
    ...
    console.warn(
      `[orchestrator] Failed to parse pipeline.config.json: ${err instanceof Error ? err.message : err}`,
    );
...
function runAgent(agentName: string, prompt: string): AgentResult {
  ...
  if (r.error)
    throw new Error(`Agent ${agentName} spawn error: ${r.error.message}`);
...
async function runAgentWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    console.warn(
      `[pipeline] transient error from ${agentName}: ${r.output.slice(0, 200) || "(empty output)"}`,
    );
...
function finishNode(
  ...
  ch(
    ...
    "0",
    String(r.costUsd || 0),
    String(r.inputTokens || 0),
    String(r.cacheWriteTokens || 0),
    String(r.cacheReadTokens || 0),
    String(r.outputTokens || 0),
    String(r.pid || 0),
  );
}
...
async function stepDesignAnalystFlow(
  threadId: string,
  flowName: string,
): Promise<{ featureSlug: string; featureDescription: string }> {
  ch(
    "start",
    threadId,
    "design_analyst_flow",
    "design-analyst-flow",
    AGENT_MODELS["design-analyst-flow"],
  );
  const r = runAgent("design-analyst-flow", `flow_name: ${flowName}`);
  const designPassed =
    !r.isError &&
    !r.output.includes("FIGMA_MCP_FAILED:") &&
    r.output.includes("feature_slug:");
  finishNode(threadId, "design_analyst_flow", r, designPassed);

  if (!designPassed) {
    const msg =
      r.output.match(/FIGMA_MCP_FAILED:(.*)/)?.[1]?.trim() ??
      (r.output.toLowerCase().includes("figma")
        ? "Figma MCP not available — ensure the MCP server is configured and running"
        : "design-analyst-flow failed — check Figma MCP");
  ...
  const featureDescription =
    r.output.match(/feature_description:\s*(.+)/)?.[1]?.trim() ?? "";
...
async function stepPlanFeature(
  ...
  if (!planOk) {
    ch(
      ...
      threadId,
      "plan_feature",
      r.output.slice(0, 500) || "plan-feature returned empty output",
    );
    ch("state", threadId, "status", "failed");
    ch(
      "state",
      threadId,
      "errorMessage",
      "plan-feature failed — check network or API key",
    );
    notify(
      "Pipeline failed",
      "plan-feature agent failed after retries.",
      "error",
      threadId,
    );
  ...
  const scopeMatch = r.output.match(
    /\*\*Scope:\*\*\s*`?(both|backend|mobile)`?/i,
  );
  const scope = scopeMatch?.[1]?.toLowerCase() ?? "both";
  ch("state", threadId, "scope", scope);

  const oqSection =
    r.output.match(/###\s*5\.\s*Open Questions([\s\S]*?)(?=###|$)/i)?.[1] ?? "";
  const openQuestions: string[] = [];
  if (!oqSection.toLowerCase().includes("no open questions")) {
    for (const m of oqSection.matchAll(/\d+\.\s*(.+)/g))
      openQuestions.push(m[1].trim());
  }
...
async function stepImplementBackend(
  ...
  ch(
    ...
    AGENT_MODELS["feature-implementation-backend"],
  );
...
async function stepImplementFrontend(
  ...
  ch(
    ...
    AGENT_MODELS["feature-implementation-frontend"],
  );
  const r = await runAgentWithRetry(
    "feature-implementation-frontend",
    `Feature: ${featureSlug}\nDescription: ${featureDescription}\n\nApproved plan:\n${plan}${extra ? "\n\n" + extra : ""}`,
...
async function stepReview(
  ...
  const r = await runAgentWithRetry(
    "reviewer",
    `Scope: ${scopeArg}. Review feature: ${featureSlug}`,
  );
  finishNode(threadId, "reviewer", r, true);

  const severity =
    r.output.match(/^REVIEW_RESULT:\s*(critical|high|clean)/m)?.[1] ?? "clean";
...
async function stepPrManager(
  ...
  if (!succeeded) {
    ...
    notify(
      "Pipeline failed",
      `PR creation failed: ${output.slice(0, 120)}`,
      "error",
      threadId,
    );
  ...
  notify(
    `Pipeline complete: ${featureSlug}`,
    "PR created. Feature marked done.",
    "info",
    threadId,
  );
...
function markTaskComplete(featureSlug: string): void {
  ...
  if (updated === original) {
    console.log(
      `[pipeline] tasks.md: no unchecked entry found for "${featureSlug}" — skipping`,
    );
    return;
  }

  writeFileSync(tasksPath, updated);
  spawnSync("git", ["add", "docs/blueprint/tasks.md"], {
    cwd: PROJECT_ROOT,
    encoding: "utf8",
  });
  const commit = spawnSync(
    "git",
    ["commit", "-m", `chore: mark ${featureSlug} complete in tasks.md`],
  ...
  if (commit.status === 0) {
    ...
    console.warn(
      `[pipeline] tasks.md: updated but git commit failed — ${commit.stderr}`,
    );
  }
}
...
async function interrupt1(
  ...
  notify(
    ...
    `Feature: ${featureDescription}. Plan ready for review.`,
    "info",
    threadId,
  );
  ...
  if (IS_TTY) {
    ...
    const qaLines = Object.entries(questionAnswers)
      .map(([q, a]) => `  Q: ${q}\n  A: ${a}`)
      .join("\n");
    console.log(`
${"━".repeat(55)}
...
Reply "approved" to proceed, or describe what needs to change.`);
    ...
    console.log(
      `[pipeline] Waiting for interrupt #1 (plan review) — ${featureDescription}`,
    );
  }
...
interface Interrupt2Result {
  decision: "merge" | "hold" | "fix";
  feedback: string;
}
...
async function interrupt2(
  ...
  notify(
    ...
    "All checks complete. Review results and reply merge, hold, or fix: <feedback>.",
    level,
    threadId,
  );
...
Reply "merge" to create the PR, "hold" to pause, or "fix: <feedback>" to re-run implementation.`);
    ...
    console.log(
      `[pipeline] Waiting for interrupt #2 (merge approval) — ${featureDescription}`,
    );
  }
  ...
  const recordType =
    decision === "merge" ? "merge" : decision === "fix" ? "feedback" : "hold";
  ch(
    "interaction-finish",
    threadId,
    "2",
  ...
  if (decision === "fix" && feedback) {
    ch(
      "feedback",
      threadId,
      "implementation_fix_request",
      featureDescription.replace(/\s+/g, "-").toLowerCase().slice(0, 40),
      "user",
...
async function executePipeline(s: State): Promise<void> {
  ...
  try {
    ...
    if (HAS_DESIGN) {
      ...
      if (design.featureDescription)
        featureDescription = design.featureDescription;
    }
    ...
    while (!planApproved) {
      ...
      if (planResult.openQuestions.length > 0) {
        questionAnswers = await resolveOpenQuestions(
          threadId,
          featureDescription,
          planResult.openQuestions,
        );
        planFeedback = ""; // don't re-show prior feedback on the re-run
        continue;
      }

      const response = await interrupt1(
        threadId,
        featureDescription,
        plan,
        questionAnswers,
    ...
    if (
      ...
      const critSection =
        reviewOutput.match(/##\s*Critical[\s\S]*?(?=##|$)/i)?.[0] ?? "";
      const highSection =
        reviewOutput.match(/##\s*High[\s\S]*?(?=##|$)/i)?.[0] ?? "";
...
async function startPipeline(
  ...
  if (!existsSync(join(PROJECT_ROOT, "docs", "blueprint", "index.md"))) {
    throw new Error(
      "Blueprint missing — run design-discovery first and check docs/blueprint/index.md",
    );
  }
  ...
  if (createResult.startsWith("EXISTING_RUN:")) {
    ...
    if (IS_TTY) {
      const ans = await askHuman(
        'Type "fresh" to abort it and start over, or press Enter to cancel:',
      );
      if (ans.toLowerCase() !== "fresh") {
  ...
  switch (state.status) {
    ...
    case "failed": {
      const failedNode = [...run.nodes]
        .reverse()
        .find((n: any) => n.status === "failed");
      console.log(
        `Pipeline ${threadId} failed at: ${failedNode?.node ?? "unknown"}`,
      );
      console.log(
        `Error: ${failedNode?.error ?? state.errorMessage ?? "unknown"}`,
      );
      return;
    }
    ...
    case "waiting_interrupt_1": {
      ...
      const openQs: string[] =
        typeof state.openQuestions === "string"
          ? JSON.parse(state.openQuestions || "[]")
          : (state.openQuestions ?? []);
      if (openQs.length > 0) {
      ...
      if (response.toLowerCase() === "approved") {
        ch("state", threadId, "status", "running");
        await continueFromImplementation(threadId, {
          ...state,
    ...
    case "running": {
      const runningNode = [...run.nodes]
        .reverse()
        .find((n: any) => n.status === "running");
      if (!runningNode) {
        console.log("No running node found.");
        return;
      }
      console.log(
        `Node '${runningNode.node}' (${runningNode.agentName}) was running when session ended.`,
      );
      ...
      switch (choice.trim()) {
        case "1":
          ...
          console.log(
            'Re-running from start not supported in resume — please use "start".',
          );
          break;
        case "2":
          ch(
...
async function rerunStep(threadId: string): Promise<void> {
  ...
  const failedNode = [...runData.nodes]
    .reverse()
    .find((n: any) => n.status === "failed");
  if (!failedNode) {
    ...
    switch (nodeName) {
      ...
      case "environment_check": {
        ...
        const designDone =
          ...
          freshNodes.some(
            (n: any) =>
              n.node === "design_analyst_flow" && n.status === "success",
          );
        if (designDone && freshState.featurePlan) {
          await continueFromImplementation(threadId, freshState);
        } else {
          console.log(
            "[pipeline] Env check passed. Run ./pipeline resume " +
              threadId +
              " to continue.",
          );
          ch("state", threadId, "status", "failed");
      ...
      default:
        console.log(
          `[pipeline] Rerun from ${nodeName} is not supported — use ./pipeline resume ${threadId}`,
        );
        ch("state", threadId, "status", "failed");
        ch(
          "state",
          threadId,
          "errorMessage",
          `Rerun not available for ${nodeName} — use ./pipeline resume`,
        );
    }
  } catch (err) {
    console.error(
...
async function continueFromImplementation(
  threadId: string,
  state: any,
  implementationFeedback = "",
): Promise<void> {
...
async function main(): Promise<void> {
  ...
  if (cmd === "start") {
    ...
    if (!flowName) {
      console.error(
        'Usage: pipeline start "<flow name>" [--scope backend|mobile|both]',
      );
      process.exit(1);
    }
    ...
    await startPipeline(
      flowName,
      scopeIdx >= 0 ? rest[scopeIdx + 1] : undefined,
    );
  } else if (cmd === "resume") {
    const threadId = rest[0];
    if (!threadId) {
      console.error("Usage: pipeline resume <threadId>");
      process.exit(1);
    }
    ...
    const threadId = rest[0];
    if (!threadId) {
      console.error("Usage: pipeline rerun <threadId>");
      process.exit(1);
    }
